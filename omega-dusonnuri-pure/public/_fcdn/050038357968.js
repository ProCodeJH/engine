$(function(){
	if(LIMIT_API_LIST.indexOf('youtube') === -1){
		...
		if($youtube_player_api.length === 0){
			$youtube_player_api = $('<script src="/_fcdn/6df251414973" id="youtube_player_api" />');
...
// Vimeo SDK 동적 로드 유틸리티 (site_video.js 에서도 사용)
const ensureVimeoSDK = (() => {
	let promise = null;
	return (callback) => {
		...
		$section_obj.imagesLoaded()
			.always(function(){
				...
				ensureVimeoSDK(() => {
					...
					tv.on('loaded', () => onPlayerReady());
					tv.on('play', () => $screen.addClass('active'));
					tv.on('ended', () => $screen.removeClass('active'));
				});
			});
